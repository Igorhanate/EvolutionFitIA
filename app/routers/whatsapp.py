...
async def _process_and_reply(
    ...
    try:
        ...
        reply = await claude_service.process_message(
            user, text, db, image_b64=image_b64, image_mimetype=image_mimetype, phone=phone
        )
        await whatsapp_service.send_message(phone, reply)
    except Exception as e:
        logger.error("background_task_error", extra={"user_id": user_id, "error": str(e)}, exc_info=True)
        try:
            ...
            logger.error("failed_to_send_error_message", extra={"user_id": user_id})
    finally:
        db.close()
...
async def whatsapp_webhook(request: Request, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if message_id:
            try:
                db.add(MensagemProcessada(message_id=message_id, user_id=user.id))
                db.commit()
