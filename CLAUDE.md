...
- [x] **Épico Parte 1 PASSO 2 ✅ CONCLUÍDO (31/05):** fluxo "treinar [nome]" — parser guard 0.8, herança do `treino_nome` em `registrar_exercicio`, bot pergunta se vier sem nome, handler de `aguardando_nome_treino` com tratamento de cancelar/comandos reservados. NÃO implementado (ficam pra Parte 1 estendida ou Parte 2): comando "finalizar" e auto-expiração 20min.
